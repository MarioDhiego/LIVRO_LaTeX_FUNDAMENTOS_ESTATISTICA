...
\textbf{(O Maior Valor de uma Imagem é quando ela nos força a notar o que nunca esperávamos ver."  - John. Tukey)} \vskip0.3cm
...
\textbf{(Um mau gráfico é pior do que nenhum gráfico.  - WALLGREN, 1996)} \vskip0.3cm
...
\section{Requisitos Fundamentais de uma Representação Gráfica}
...
\newpage
\section{Diretrizes e Recomendações para a Construção de um Gráfico}
...
\section{Sugestões de Leitura e Interpretação de um Gráfico}
...
\newpage
\section{Principais Erros Encontrados em um Gráfico}
...
\includegraphics[scale=0.40]{figures/histograma1.jpeg}
    \caption{(\textbf{Histograma}:) das Idades de Vítimas Fatais por Acidentes de Trânsito, Belém em 2022.}
...
\newpage
\subsection{Gráfico Circular, Setorial ou Pizza (\textit{Pie Chart})}
...
\newpage
\subsubsection{Gráfico de Rosca (\textit{Donut Chart})}

Um gráfico de rosca ou rosquinha (\textit{donut chart}, em inglês) é uma variação de um gráfico de pizza padrão. A grande diferença entre eles é a presença de um furo no meio.\vskip0.3cm

Os gráficos de rosca são os primos mais jovens e descolados do gráfico de pizza, e é fácil perceber por quê. Eles são elegantes e simples e se ajustarão a praticamente qualquer estilo de design. São uma versão moderna de um gráfico tradicional. \vskip0.3cm


Um gráfico de rosca geralmente mostra as proporções de dados categóricos em que o tamanho de cada pedaço da rosca comunica a proporção de cada categoria.\vskip0.3cm


Os gráficos de rosca também podem ser cortados ao meio para economizar ainda mais espaço, chamados de meia rosca. 

\begin{figure}[H]
    \centering
\includegraphics[scale=0.25,height=200pt,width=11cm]{figures/Rosca2.jpeg}
    \caption{\textbf{(Gráfico Meia Rosca):} Condição das Vítimas Não Fatal por Acidentes de Trânsito, Registrados no Município de Belém, Estado do Pará, no período de 2016 a 2022.}
