...
\inic Tabela é a forma não discursiva de apresentação de informações, representadas por dados numéricos e codificações, dispostos em uma ordem determinada, segundo as variáveis analisadas de um fenômeno.\vskip0.3cm


Segundo Milone(2004), toda tabela deve ser \textbf{Simples}, \textbf{Clara}, \textbf{Objetiva} e \textbf{Auto-Explicativa}.
...
\section{Elementos Básicos na Construção Tabular}
...
Segue um exemplo com dados reais referentes a População Estimada Total do Estado do Pará, para ilustrar o esquema geral sobre uma construção tabular simples, conforme a figura \ref{amostras estratificada}.
...
\newpage
\section{Quadros Estatístico}

\inic Os quadros são definidos como arranjo predominante de palavras dispostas em linhas e colunas, com ou sem indicação de dados numéricos. È formado por linhas horizontais e verticais, sendo, portanto "fechado", e normalmente, é usado para apresentar dados secundários, e geralmente vem no referencial teórico do trabalho.\vskip0.3cm 
...
\newpage
\section{Regras para Construção Tabular}
...
\begin{enumerate}
  ...
  \item \textbf{Amplitude Total da Distribuição de Freqüência (AT)}: é a diferença existente entre os limites reais superior(maior) e inferior (menor) valor observado da distribuição de freqüência;
